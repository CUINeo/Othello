...
class MctsEngine(Engine):
    ...
    def get_move(self, board, color, move_num=None,
        ...
        # If there are corner moves, return them
        if (0, 0) in self.root.remain_valid_moves:
            return (0, 0)
        if (0, 7) in self.root.remain_valid_moves:
            return (0, 7)
        if (7, 0) in self.root.remain_valid_moves:
            return (7, 0)
        if (7, 7) in self.root.remain_valid_moves:
            return (7, 7)
        # Get the move number
    ...
    def terminal_reward(self, board):
        ...
        #     for entry in line:
        #         reward += entry
        reward = sum(map(sum, board.pieces))
        if reward > 0:
            return 1
        elif reward < 0:
            return 0
    ...
    def default_policy(self, node):
        ...
        while remaining_num > 0:
            while cur_move_num < 64:
                # Check corner number
                corner_num = cur_board.pieces[0][0] + cur_board.pieces[0][7] + cur_board.pieces[7][0] + cur_board.pieces[7][7]
                if corner_num >= 1:
                    # White possesses more corner nodes
                    cur_reward += 1
                    break
                elif corner_num <= -1:
                    # Black possesses more corner nodes
                    break
